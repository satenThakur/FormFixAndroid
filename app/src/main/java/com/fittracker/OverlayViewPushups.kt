...
class OverlayViewPushups(context: Context?, attrs: AttributeSet?) :
    ...
    private var kneeNewAngle = 0
    private var hipNewAngle = 0
    ...
    private fun initPaints() {
        ...
        pointErrorPaint.strokeWidth = Constants.ERROR_STROKE_WIDTH
    ...
    override fun draw(canvas: Canvas) {
        ...
        results?.let { poseLandmarkResult ->
            ...
            if (!isTimerCompleted)
                return
            Utility.Log("NOSEY", "yForNose=$yForNose")
            ...
            if (xHeel > 0 && yHeel > 0 && userFaceType == Constants.FRONT_FACE) {
                ...
                when (Utility.getState(kneeAngle, hipAngle, Constants.FRONT_FACE)) {
                    STATE_MOVING -> {
                        ...
                        if (hipAngle < 100 && hipAngle > 90 && !statesSet.contains(STATE_DOWN)) {
                            speakOut(resources.getString(R.string.squat_down_further))
                        }
                    }

                    STATE_UP -> {
                        Utility.Log("STATE_UP", "for FRONT")
                        canvas.drawText(
                    ...
                    STATE_DOWN -> {
                        ...
                        Utility.Log(
                            "diff_toesX",
                            "" + abs((xofRightToe - xofLeftToe) * imageWidth * scaleFactor)
                        )
                        Utility.Log(
                            "diff_kneesX",
                            "" + abs((xofRightKnee - xofLeftKnee) * imageWidth * scaleFactor)
                        )
                        Utility.Log(
                            "diff____",
                            "" + abs(((xofRightKnee - xofLeftKnee) - (xofRightToe - xofLeftToe)) * imageWidth * scaleFactor)
                        )
                        if (heelAngle > 0) {
                            if (heelAngle < HEEL_MIN_ANGLE) {
                                Utility.Log("ErrorMessage", "3 heelAngle=" + heelAngle)
                                isHeelCorrect = false
                            ...
                            if (HEEL_MIN_ANGLE <= heelAngle || heelAngle <= HEEL_MAX_ANGLE) {
                                isHeelCorrect = true
                                var diff =
                                    abs(xofLeftKnee - xofRightKnee) - abs(xofLeftToe - xofRightToe)
                                diff = diff * imageHeight * scaleFactor

                                Utility.Log("ErrorMessage", "4 kneex toex diff=" + diff)
                                if (diff < -TOE_KNEE_X_DIFFS_MIN_THRESHOLD) {
                                    isFrontFaceErrorMessage = true;
                                    Utility.Log("ErrorMessage", "4 kneex toex diff=_true" + diff)
                ...
                when (Utility.getState(kneeAngle, hipAngle, Constants.LEFT_FACE)) {
                    STATE_MOVING -> {
                        ...
                        if (hipAngle < 85 && kneeAngle > 110 && !statesSet.contains(STATE_DOWN)) {
                    ...
                    STATE_DOWN -> {
                        ...
                        if (Utility.isKneeCrossesToes(
                            ...
                            Utility.Log("ErrorMessage", "3 kneeCrossingToes=" + abs(toeX - xKnee))
                            canvas.drawCircle(
                        ...
                        Utility.Log("parallelAngles", "normal diff=" + abs(kneeAngle - hipAngle))
                        if (Utility.kneeHipAnglesDiff(
                            ...
                            Utility.Log(
                                "ErrorMessage",
                                "4 tuckHips=" + abs(kneeAngle - hipAngle) + " kneeAngle=" + kneeAngle + " hipAngle=" + hipAngle
                            )
                            canvas.drawCircle(
    ...
    private fun onMaskResultScaled(mpImage: MPImage) {
        ...
        Utility.Log("segment", " h=" + mpImage.height + " w=" + mpImage.width)
        Utility.Log("segment", " scaleHeight=" + scaleHeight + " scaleWidth=" + scaleWidth)
        invalidate()
    }
